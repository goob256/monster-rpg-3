...
static Image *pad(Image *image)
{
	...
	for (int y = 0; y < tiles_h; y++) {
		for (int x = 0; x < tiles_w; x++) {
			int sx = x * shim::tile_size;
			int sy = y * shim::tile_size;
			int dx = x * (shim::tile_size + 2) + 1;
			int dy = y * (shim::tile_size + 2) + 1;
			image->draw_region(
				util::Point<int>(sx, sy),
				util::Size<int>(shim::tile_size, shim::tile_size),
				util::Point<int>(dx, dy)
			);
			// sides
			image->draw_region(
				util::Point<int>(sx, sy),
				util::Size<int>(shim::tile_size, 1),
				util::Point<int>(dx, dy-1)
			);
			image->draw_region(
				util::Point<int>(sx, sy),
				util::Size<int>(1, shim::tile_size),
				util::Point<int>(dx-1, dy)
			);
			image->draw_region(
				util::Point<int>(sx, sy+shim::tile_size-1),
				util::Size<int>(shim::tile_size, 1),
				util::Point<int>(dx, dy+shim::tile_size)
			);
			image->draw_region(
				util::Point<int>(sx+shim::tile_size-1, sy),
				util::Size<int>(1, shim::tile_size),
				util::Point<int>(dx+shim::tile_size, dy)
			);
			// corners
			image->draw_region(
				util::Point<int>(sx, sy),
				util::Size<int>(1, 1),
				util::Point<int>(dx-1, dy-1)
			);
			image->draw_region(
				util::Point<int>(sx+shim::tile_size-1, sy),
				util::Size<int>(1, 1),
				util::Point<int>(dx+shim::tile_size, dy-1)
			);
			image->draw_region(
				util::Point<int>(sx+shim::tile_size-1, sy+shim::tile_size-1),
				util::Size<int>(1, 1),
				util::Point<int>(dx+shim::tile_size, dy+shim::tile_size)
			);
			image->draw_region(
				util::Point<int>(sx, sy+shim::tile_size-1),
				util::Size<int>(1, 1),
				util::Point<int>(dx-1, dy+shim::tile_size)
			);
		}
	}
	...
	for (int layer = 0; layer < num_layers; layer++) {
		...
		for (int row = 0; row < size.h; row++) {
			...
			for (int col = 0; col < size.w; col++) {
				...
				unsigned char y = util::SDL_fgetc(f);
				if (layer == 0) {
					solid[row][col] = (y & 0x80) ? true : false;
					y = y & 0x7f;
				}
				layers[layer].y[row][col] = y;
			}
		}
	}
...
void Tilemap::save(std::string filename)
{
	...
	for (int row = 0; row < size.h; row++) {
		for (int col = 0; col < size.w; col++) {
			for (int layer = 0; layer < num_layers; layer++) {
				if (layers[layer].x[row][col] != 255) {
					if (all_clear(layers[layer].x[row][col], layers[layer].y[row][col])) {
						layers[layer].x[row][col] = 255;
						layers[layer].y[row][col] = 0;
					}
				}
			}
		}
	...
	for (int layer = 0; layer < num_layers; layer++) {
		for (int row = 0; row < size.h; row++) {
			for (int col = 0; col < size.w; col++) {
				...
				unsigned char y = layers[layer].y[row][col];
				// store solid flag in high bit of y in first layer
				if (layer == 0) {
					if (is_solid(-1, util::Point<int>(col, row))) {
						y |= 0x80;
					}
				}
				util::SDL_fputc(y, file);
			}
		}
	}
