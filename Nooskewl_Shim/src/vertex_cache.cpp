...
void Vertex_Cache::start(Image_Base *image, bool repeat)
{
	*_image = image == 0 ? 0 : image->get_root();
	*_repeat = repeat;
	*_started = true;
}
...
void Vertex_Cache::cache(SDL_Colour vertex_colours[4], util::Point<float> source_position, util::Size<float> source_size, util::Point<float> da, util::Point<float> db, util::Point<float> dc, util::Point<float> dd, int flags)
{
	...
	if (*_image) {
		...
		if (flags & Image_Base::FLIP_H) {
			float tmp = tu;
			tu = tu2;
			tu2 = tmp;
		}
		if (flags & Image_Base::FLIP_V) {
			float tmp = tv;
			tv = tv2;
			tv2 = tmp;
		}
