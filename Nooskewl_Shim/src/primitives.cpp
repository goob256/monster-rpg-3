...
static void draw_line_worker(SDL_Colour colour, noo::util::Point<float> a, noo::util::Point<float> b, float thickness)
{
	...
	noo::gfx::Vertex_Cache::instance()->cache(vertex_colours, noo::util::Point<float>(0, 0), noo::util::Size<float>(0, 0), da, dc, dd, db, 0);
}
...
static void draw_straight_line_worker(SDL_Colour colour, noo::util::Point<float> a, noo::util::Point<float> b, float thickness)
{
	...
	noo::gfx::Vertex_Cache::instance()->cache(vertex_colours, noo::util::Point<float>(0, 0), noo::util::Size<float>(0, 0), da, db, dc, dd, 0);
}
...
void draw_filled_ellipse(SDL_Colour colour, util::Point<float> centre, float rx, float ry, int sections)
{
	...
	for (int n = 0; n < sections; n++) {
		int n2 = (n+1) % sections;
		float a1 = (float)n/sections * (float)M_PI * 2.0f;
		float a2 = (float)n2/sections * (float)M_PI * 2.0f;
...
void draw_ellipse(SDL_Colour colour, util::Point<float> centre, float rx, float ry, float thickness, int sections)
{
	SDL_Colour colours[3] = { colour, colour, colour };
...
void draw_filled_circle(SDL_Colour colour, util::Point<float> centre, float radius, int sections)
{
	draw_filled_ellipse(colour, centre, radius, radius, sections);
}

void draw_circle(SDL_Colour colour, util::Point<float> centre, float radius, float thickness, int sections)
{
	draw_ellipse(colour, centre, radius, radius, thickness, sections);
}
...
void draw_filled_rectangle(SDL_Colour vertex_colours[4], util::Point<float> dest_position, util::Size<float> dest_size)
{
	...
	Vertex_Cache::instance()->cache(vertex_colours, util::Point<float>(0, 0), util::Size<float>(0, 0), dest_position, dest_size, 0);
	if (prim_held == false) {
		draw_primitives_end();
	}
}
