...
#include "Nooskewl_Shim/tilemap.h"
...
#include "Nooskewl_Shim/shaders/glsl/model_vertex.h"
#include "Nooskewl_Shim/shaders/glsl/model_fragment.h"

#ifdef _WIN32
#include "Nooskewl_Shim/shaders/hlsl/default_vertex.h"
#include "Nooskewl_Shim/shaders/hlsl/default_fragment.h"
#include "Nooskewl_Shim/shaders/hlsl/default_textured_fragment.h"
#include "Nooskewl_Shim/shaders/hlsl/model_fragment.h"
#endif
...
static void create_window(int scaled_w, int scaled_h, bool force_integer_scaling, int &window_w, int &window_h)
{
	...
	internal::gfx_context.fullscreen = util::bool_arg(false, shim::argc, shim::argv, "fullscreen");
	vsync = util::bool_arg(true, shim::argc, shim::argv, "vsync");
	show_fps = util::bool_arg(false, shim::argc, shim::argv, "fps");
	...
	if (window_w < 0 && window_h < 0) {
		...
		if (scaled_w < 0 || scaled_h < 0) {
			...
			internal::gfx_context.fullscreen = false;
			int n = get_max_comfortable_scale(util::Size<int>(scaled_w, scaled_h)); // guesses 1920x1080
			window_w = n * scaled_w;
			window_h = n * scaled_h;
		}
			...
			int n = get_max_comfortable_scale(util::Size<int>(scaled_w, scaled_h));
			window_w = n * scaled_w;
			window_h = n * scaled_h;
		}
	}
	...
	bool is_cb = env->CallBooleanMethod(activity, method_id);
	...
	shim::real_screen_size = util::Size<int>(w, h);
}
...
#ifdef _WIN32
	if (icon_small != 0) {
		SetClassLong(internal::gfx_context.hwnd, GCL_HICONSM, 0L);
		DestroyIcon(icon_small);
	}
	if (icon_big != 0) {
		SetClassLong(internal::gfx_context.hwnd, GCL_HICON, 0L);
...
static void set_window_icon()
{
	...
	try {
		...
		if (SystemParametersInfo(SPI_GETWORKAREA, 0, &r, 0)) {
			...
			if (GetVersionEx(&vi) && vi.dwMajorVersion == 6 && std::abs(40-taskbar_h) <= 5) {
				o = 0;
			}
			else {
	...
	SetClassLong(internal::gfx_context.hwnd, GCL_HICON, (LONG)icon_big);
#endif
...
#if ((defined __APPLE__ && !defined IOS) || (defined __linux__ && !defined ANDROID && !defined RASPBERRYPI) || defined _WIN32)
static void create_mouse_cursors()
{
...
static void real_set_scissor(int x, int y, int w, int h)
{
	if (shim::opengl) {
		int szy;
...
#ifdef USE_TTF
			shim::font = new TTF("hfont.ttf", 8*int(shim::scale));
#else
			shim::font = new Pixel_Font("hfont");
#endif
		}
		else {
...
void static_start()
{
	...
	minimum_window_size = util::Size<int>(-1, -1);
	maximum_window_size = util::Size<int>(-1, -1);

	press_and_hold_state = -1;

	last_gui_size = util::Size<int>(-1, -1);
	...
	last_screen_mode = util::Size<int>(-1, -1);
...
// This functions "pauses" events (on Linux SDL can spam keys when resizing/recreating windows)
...
void register_lost_device_callbacks(void (*lost)(), void (*found)())
{
...
void set_screen_size(util::Size<int> size)
{
	...
	if (size.w == scaled_w && size.h == scaled_h) {
		...
		size = util::Size<int>(int(inv * size.w), int(inv * size.h));
	...
	if (shim::allow_dpad_below && shim::screen_size.h*shim::scale*1.5f < orig_size.h) {
		shim::screen_offset = util::Point<int>(int(orig_size.w-(shim::screen_size.w*shim::scale))/2, int(orig_size.h-(shim::screen_size.h*1.5f*shim::scale))/2); // room for controls
		shim::dpad_below = true;
	}
	else {
		shim::screen_offset = util::Point<int>(int(orig_size.w-(shim::screen_size.w*shim::scale))/2, int(orig_size.h-(shim::screen_size.h*shim::scale))/2);
...
void draw_notifications()
{
	if (notifications.size() > 0) {
		...
		if (shim::font->get_text_width(text) > shim::screen_size.w * 0.9f) {
			...
			draw_filled_rectangle(colour, util::Point<int>(x - 3, y - 3), util::Size<int>(w + 6, h + 6));
			colour.r = colour.g = colour.b = alpha;

			fancy_draw(colour, line1, util::Point<int>(x, y));
			fancy_draw(colour, line2, util::Point<int>(x, y+shim::font->get_height()));
		}
		else {
			draw_filled_rectangle(colour, util::Point<int>(x - 3, y - 3), util::Size<int>(w + 6, h + 6));
			colour.r = colour.g = colour.b = alpha;
			fancy_draw(colour, text, util::Point<int>(x, y));
		}
...
void flip()
{
	...
	if (internal::gfx_context.fullscreen && internal::gfx_context.restarting == false) {
		...
		if (flags & SDL_WINDOW_INPUT_FOCUS) {
			...
			if (internal::My_SDL_GetCurrentDisplayMode(shim::adapter, &m) == 0) {
				...
				last_screen_mode = util::Size<int>(m.w, m.h);
			}
		}
	}
#endif
...
#endif
	{
		if (show_fps && shim::font != 0) {
			...
			shim::font->draw(shim::white, util::itos(fps), util::Point<float>(2.0f, 2.0f));
...
void draw_9patch_tinted(SDL_Colour tint, Image *image, util::Point<int> dest_position, util::Size<int> dest_size)
{
	int w = image->size.w;
	int size = image->size.w / 3;
	util::Size<int> dim(size, size);

	image->start_batch();

	// Corners
	image->draw_region_tinted(tint, util::Point<int>(0, 0), dim, dest_position); // top left
	image->draw_region_tinted(tint, util::Point<int>(w-size, 0), dim, util::Point<int>(dest_position.x+dest_size.w-size, dest_position.y)); // top right
	image->draw_region_tinted(tint, util::Point<int>(w-size, w-size), dim, dest_position+dest_size-dim); // bottom right
	image->draw_region_tinted(tint, util::Point<int>(0, w-size), dim, util::Point<int>(dest_position.x, dest_position.y+dest_size.h-size)); // bottom left

	// Sides
	image->stretch_region_tinted_repeat(tint, util::Point<int>(size, 0), dim, util::Point<int>(dest_position.x+size, dest_position.y), util::Size<int>(dest_size.w-size*2, size)); // top
	image->stretch_region_tinted_repeat(tint, util::Point<int>(0, size), dim, util::Point<int>(dest_position.x, dest_position.y+size), util::Size<int>(size, dest_size.h-size*2)); // left
	image->stretch_region_tinted_repeat(tint, util::Point<int>(w-size, size), dim, util::Point<int>(dest_position.x+dest_size.w-size, dest_position.y+size), util::Size<int>(size, dest_size.h-size*2)); // right
	image->stretch_region_tinted_repeat(tint, util::Point<int>(size, w-size), dim, util::Point<int>(dest_position.x+size, dest_position.y+dest_size.h-size), util::Size<int>(dest_size.w-size*2, size)); // bottom

	// Middle
	image->stretch_region_tinted_repeat(tint, util::Point<int>(size, size), dim, dest_position+dim, dest_size-dim*2);
...
void fancy_draw(SDL_Colour colour, std::string text, util::Point<int> position)
{
	...
	if (t < 1000 || count < 2) {
		...
		for (int i = 0; i < util::utf8_len(text); i++) {
			...
			shim::font->draw(colour, c, util::Point<float>(position.x + dx, (float)position.y));
			curr_s += c;
		}
	}
}
...
void load_palette(std::string name)
{
	...
	while (count < sz && util::SDL_fgets(file, line, 1000) != 0) {
		count += strlen(line);
		line_count++;
		char *p = line;
		...
		if (sscanf(line, "%d %d %d", &red, &green, &blue) == 3) {
			shim::palette[colour_count].r = red;
			shim::palette[colour_count].g = green;
			shim::palette[colour_count].b = blue;
			shim::palette[colour_count].a = 255;
			colour_count++;
			if (colour_count >= 256) {
				break;
			}
...
{
	int sx, sy;
	if (internal::gfx_context.target_image == internal::gfx_context.work_image || internal::gfx_context.target_image == 0) {
		sx = scissor_x;
		sy = scissor_y;
	}
	else {
...
util::Size<int> get_desktop_resolution()
{
	...
	for (int i = 0; i < num; i++) {
		...
		if ((mode.w > size.w && mode.h > size.h) || (mode.w == size.w && mode.h > size.h) || (mode.w > size.w && mode.h == size.h)) {
			size = util::Size<int>(mode.w, mode.h);
		}
	}
...
int get_max_comfortable_scale(util::Size<int> scaled_size)
{
	...
	int w = int(mode.w * 0.875f);
	int h = int(mode.h * 0.875f);
...
void resize_window(int width, int height)
{
	...
	shim::real_screen_size = util::Size<int>(width, height);
