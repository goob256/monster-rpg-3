...
int main(void)
{
	...
	ttf = al_load_font("font.ttf", 8, ALLEGRO_TTF_MONOCHROME | ALLEGRO_TTF_NO_KERNING | ALLEGRO_TTF_NO_AUTOHINT);
	...
	for (int i = 0; i < num_glyphs; i++) {
		...
		al_draw_text(
			...
			1+y * (SIZE + PAD*2) + PAD, // extra +2 for first row
			0,
			s.c_str()
		);
		int w = SIZE + PAD*2;
		...
		al_draw_text(
			...
			al_map_rgb(255, 255, 255),
			(x * (SIZE + PAD*2)) * 3 + PAD + w + 1 + xo,
			1+y * (SIZE + PAD*2) + PAD + 1, // extra +2 for first row
			0,
			s.c_str()
		);
		al_draw_text(
