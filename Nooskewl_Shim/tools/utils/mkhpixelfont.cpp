...
int main(void)
{
	...
	ttf = al_load_font("font.ttf", 32, /*ALLEGRO_TTF_MONOCHROME |*/ ALLEGRO_TTF_NO_KERNING | ALLEGRO_TTF_NO_AUTOHINT);
	ttf_noaa = al_load_font("font.ttf", 32, ALLEGRO_TTF_MONOCHROME | ALLEGRO_TTF_NO_KERNING | ALLEGRO_TTF_NO_AUTOHINT);
	...
	info_txt++;
	int SIZE = atoi((const char *)info_txt);

	ALLEGRO_BITMAP *bmp = al_create_bitmap((half * (SIZE + PAD*2)) * 3, half * (SIZE + PAD*2));
		...
		for (int yyy = 0; yyy <= 4; yyy++) {
			for (int xxx = 0; xxx <= 4; xxx++) {
				al_draw_text(
					ttf_noaa,
					al_map_rgb(255, 255, 255),
					(x * (SIZE + PAD*2)) * 3 + PAD + w + xxx + xo,
					y * (SIZE + PAD*2) + PAD + yyy,
					0,
					s.c_str()
				);
			}
		...
		for (int yyy = -4; yyy <= 4; yyy++) {
			for (int xxx = -4; xxx <= 4; xxx++) {
				al_draw_text(
					ttf_noaa,
					al_map_rgb(255, 255, 255),
					(x * (SIZE + PAD*2)) * 3 + PAD + w*2 + xxx + xo,
					y * (SIZE + PAD*2) + PAD + yyy, // extra +2 for first row
					0,
					s.c_str()
				);
			}
